...
static inline struct tcp_socket *port_hash_search(u32 saddr, u16 sport)
{
	...
	hlist_for_each_entry(ts, &port_hash[bucket], link)
	    if (ts->sport == sport &&
		(saddr == INADDR_ANY
		 || ts->saddr == saddr || ts->saddr == INADDR_ANY))
		return ts;
...
static void rt_tcp_retransmit_handler(void *data)
{
	...
	if (ts->timer_state) {
		...
		skb =
		    rtskb_clone(ts->retransmit_queue.first, &ts->sock.skb_pool);
		rtdm_lock_put_irqrestore(&ts->socket_lock, context);
	...
	if (th->ack) {
		...
		if (rt_tcp_before(ts->sync.seq + 1, ntohl(th->ack_seq))) {
			rtdm_printk("rttcp: unexpected ACK %u %u %u\n",
				    ts->sync.seq,
				    ts->nacked_first, ntohl(th->ack_seq));
			rtdm_lock_put_irqrestore(&ts->socket_lock, context);
			goto drop;
		}
 ...
 *  rt_tcp_connect
 */
static int rt_tcp_connect(struct tcp_socket *ts,
			  const struct sockaddr *serv_addr, socklen_t addrlen)
{
	...
	ret =
	    rtdm_event_timedwait(&ts->conn_evt, rt_tcp_connection_timeout,
				 NULL);
	if (unlikely(ret < 0))
		switch (ret) {
		case -EWOULDBLOCK:
...
static ssize_t rt_tcp_read(struct rtdm_fd *fd, void *buf, size_t nbyte)
{
	...
	while (copied < nbyte) {
		ret =
		    rtdm_sem_timeddown(&ts->sock.pending_sem, timeout,
				       &timeout_seq);
...
iterate_fragments:
		...
		if (copied > nbyte) {
			...
			if (rtdm_copy_to_user
			    (fd, user_buf, skb->data, block_size)) {
