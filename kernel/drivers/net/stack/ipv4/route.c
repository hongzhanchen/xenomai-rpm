...
static struct host_route
    host_routes[CONFIG_XENO_DRIVERS_NET_RTIPV4_HOST_ROUTES];
			 ...
			 struct rtnet_device *rtdev)
{
	...
	if (new_route) {
		...
		rtdm_lock_put_irqrestore(&host_table_lock, context);
		 ...
		 /*ERRMSG*/
		    rtdm_printk("RTnet: no more host routes available\n");
		ret = -ENOBUFS;
	}
	...
	if (new_route) {
		...
		*last_ptr = new_route;
		...
		rtdm_lock_put_irqrestore(&net_table_lock, context);
		 ...
		 /*ERRMSG*/
		    rtdm_printk("RTnet: no more network routes available\n");
		return -ENOBUFS;
	}
}
...
int rt_ip_route_forward(struct rtskb *rtskb, u32 daddr)
{
	...
	if (likely
	    ((daddr == rtdev->local_ip) || (daddr == rtdev->broadcast_ip)
	     || (rtdev->flags & IFF_LOOPBACK)))
		return 0;

	if (rtskb_acquire(rtskb, &global_pool) != 0) {
		 /*ERRMSG*/
		    rtdm_printk("RTnet: router overloaded, dropping packet\n");
		goto error;
	}
