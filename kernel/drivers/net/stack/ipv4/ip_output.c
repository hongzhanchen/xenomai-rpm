...
int rt_ip_build_xmit_slow(struct rtsocket *sk,
			  int getfrag(const void *, char *, unsigned int,
				      unsigned int), const void *frag,
			  unsigned length, struct dest_route *rt, int msg_flags,
...
#define FRAGHEADERLEN sizeof(struct iphdr)
	...
	for (offset = 0; offset < length; offset += fragdatalen) {
		...
		iph->check =
		    ip_fast_csum((unsigned char *)iph, 5 /*iph->ihl */ );

		if ((err =
		     getfrag(frag, ((char *)iph) + 5 /*iph->ihl */  * 4, offset,
			     fraglen - FRAGHEADERLEN)))
			goto error;

		if (rtdev->hard_header) {
			err =
			    rtdev->hard_header(skb, rtdev, ETH_P_IP,
