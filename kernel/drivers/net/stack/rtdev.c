...
int rtdev_init(struct rtnet_device *rtdev, unsigned dev_pool_size)
{
	...
	ret =
	    rtskb_pool_init(&rtdev->dev_pool, dev_pool_size, &rtdev_ops, rtdev);
...
int __rt_init_etherdev(struct rtnet_device *rtdev,
		       unsigned dev_pool_size, struct module *module)
{
	int ret;
...
int rtdev_up(struct rtnet_device *rtdev, struct rtnet_core_cmd *cmd)
{
	...
	if ((rtdev->flags & IFF_UP) &&
	    (((cmd->args.up.set_dev_flags | cmd->args.up.clear_dev_flags) &
	      IFF_PROMISC) || (cmd->args.up.dev_addr_type != ARPHRD_VOID))) {
		ret = -EBUSY;
		goto out;
	}
	...
	if (ret == 0) {
		...
		list_for_each(entry, &event_hook_list) {
			hook =
			    list_entry(entry, struct rtdev_event_hook, entry);
			if (hook->ifup)
				hook->ifup(rtdev, cmd);
		}
