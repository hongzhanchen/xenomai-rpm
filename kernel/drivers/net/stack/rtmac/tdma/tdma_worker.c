					 ...
					 rtdm_lockctx_t lockctx)
{
	...
	if (job->reply_cycle == tdma->current_cycle) {
		...
		rtdm_task_sleep_abs(tdma->current_cycle_start +
				    job->reply_offset, RTDM_TIMERMODE_REALTIME);
...
void tdma_worker(void *arg)
{
	...
	while (!rtdm_task_should_stop()) {
		...
		case XMIT_REQ_CAL:
			job =
			    do_request_cal_job(tdma, REQUEST_CAL_JOB(job),
					       lockctx);
			break;
		...
		case XMIT_RPL_CAL:
			job =
			    do_reply_cal_job(tdma, REPLY_CAL_JOB(job), lockctx);
			break;
