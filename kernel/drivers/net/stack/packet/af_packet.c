...
rt_packet_recvmsg(struct rtdm_fd *fd, struct user_msghdr *u_msg, int msg_flags)
{
	...
	if (msg->msg_name) {
		...
		namelen = sizeof(sll);
		ret =
		    rtnet_put_arg(fd, &u_msg->msg_namelen, &namelen,
				  sizeof(namelen));
		if (ret)
			goto fail;
	}
	...
	if (copy_len > len) {
		...
		ret =
		    rtnet_put_arg(fd, &u_msg->msg_flags, &flags, sizeof(flags));
		if (ret)
			goto fail;
	}

	copy_len =
	    rtnet_write_to_iov(fd, iov, msg->msg_iovlen, rtskb->data, copy_len);
out:
 ...
 */
static ssize_t
rt_packet_sendmsg(struct rtdm_fd *fd, const struct user_msghdr *msg,
		  int msg_flags)
{
	struct rtsocket *sock = rtdm_fd_to_private(fd);
	size_t len;
	...
	ret =
	    rtnet_read_from_iov(fd, iov, msg->msg_iovlen, rtskb_put(rtskb, len),
				len);
