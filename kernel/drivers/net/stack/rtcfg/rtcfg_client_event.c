...
static void rtcfg_client_recv_stage_1(int ifindex, struct rtskb *rtskb)
{
	...
	case RTCFG_ADDR_IP:{
			...
			if (rtskb->len < sizeof(struct rtcfg_frm_stage_1_cfg) +
				...
				rtdm_mutex_unlock(&rtcfg_dev->dev_mutex);
				RTCFG_DEBUG(1,
					    "RTcfg: received invalid stage_1_cfg "
					    "frame\n");
				kfree_rtskb(rtskb);
				return;
			}
			...
			if (rtskb->pkt_type == PACKET_BROADCAST) {
				...
				if (rtdev->flags & IFF_BROADCAST)
					rt_ip_route_add_host(bcast,
							     rtdev->broadcast,
							     rtdev);
			}

			/* update routing table */
			rt_ip_route_add_host(saddr,
				     ...
				     u8 * mac_addr, u8 flags)
{
	if (rtcfg_dev->stations_found == rtcfg_dev->spec.clt.max_stations) {
		RTCFG_DEBUG(1,
			    "RTcfg: insufficient memory for storing new station "
			    "address\n");
		return -ENOMEM;
	}

	/* Ethernet-specific! */
	memcpy(&rtcfg_dev->spec.
	       clt.station_addr_list[rtcfg_dev->stations_found].mac_addr,
	       mac_addr, ETH_ALEN);
...
static void rtcfg_client_recv_stage_2_cfg(int ifindex, struct rtskb *rtskb)
{
	...
	if (stage_2_cfg->heartbeat_period) {
		ret =
		    rtdm_timer_init(&rtcfg_dev->timer, rtcfg_timer,
				    "rtcfg-timer");
		if (ret == 0) {
		...
		if (ret < 0)
			/*ERRMSG*/
			    rtdm_printk("RTcfg: unable to create timer task\n");
		else
	...
	for (i = 0; i < rtcfg_dev->stations_found; i++)
		...
		if (memcmp(rtcfg_dev->spec.clt.station_addr_list[i].mac_addr,
			...
			if ((rtcfg_dev->spec.clt.station_addr_list[i].flags &
			     _RTCFG_FLAG_READY) == 0) {
				rtcfg_dev->spec.clt.station_addr_list[i].
				    flags |= _RTCFG_FLAG_READY;
				rtcfg_dev->stations_ready++;
			}
			break;
		}
