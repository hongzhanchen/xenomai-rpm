...
rtcfg_main_state_off,
	    ...
	    rtcfg_main_state_client_2, rtcfg_main_state_client_ready};
	...
	if (cmd_event->args.add.stage2_file != NULL) {
		if (cmd_event->args.add.stage2_file->buffer != NULL) {
			...
			new_conn->stage2_file =
			    rtcfg_get_file(cmd_event->args.add.stage2_file->
					   name);
				      ...
				      struct rtskb *rtskb)
{
	...
	list_for_each(entry, &rtcfg_dev->spec.srv.conn_list) {
		...
		case RTCFG_ADDR_MAC:
			/* Ethernet-specific! */
			if (memcmp
			    (conn->mac_addr, rtskb->mac.ethernet->h_source,
			     ETH_ALEN) == 0) {
				rtcfg_do_conn_event(conn, event_id, rtskb);
