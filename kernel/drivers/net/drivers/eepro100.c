...
MODULE_PARM_DESC(options,
		 "eepro100: Bits 0-3: tranceiver type, bit 4: full duplex, bit 5: 100Mbps");
...
MODULE_PARM_DESC(txdmaccount,
		 "eepro100 Tx DMA burst length; 128 - disable (0-128)");
MODULE_PARM_DESC(rxdmaccount,
		 "eepro100 Rx DMA burst length; 128 - disable (0-128)");
MODULE_PARM_DESC(max_interrupt_work,
		 "eepro100 maximum events handled per interrupt");
MODULE_PARM_DESC(multicast_filter_limit,
		 "eepro100 maximum number of filtered multicast addresses");
	...
	while (inb(cmd_ioaddr) != 0) {
		if (wait-- == 0) {
			rtdm_printk(KERN_ALERT
				    "eepro100: rt_wait_for_cmd_done(%s) "
				    "timeout!\n", cmd);
			return 1;
		}
...
enum SCBCmdBits {
	SCBMaskCmdDone = 0x8000, SCBMaskRxDone = 0x4000, SCBMaskCmdIdle =
	    0x2000,
	SCBMaskRxSuspend = 0x1000, SCBMaskEarlyRx = 0x0800, SCBMaskFlowCtl =
	    0x0400,
	SCBTriggerIntr = 0x0200, SCBMaskAll = 0x0100,
	/* The rest are Rx and Tx commands. */
	CUStart = 0x0010, CUResume = 0x0020, CUStatsAddr = 0x0040, CUShowStats =
	    0x0050,
	CUCmdBase = 0x0060,	/* CU Base address (set to zero) . */
	CUDumpStats = 0x0070,	/* Dump then reset stats counters. */
	RxStart = 0x0001, RxResume = 0x0002, RxAbort = 0x0004, RxAddrLoad =
	    0x0006,
...
enum RxFD_bits {
	...
	RxErrCRC = 0x0800, RxErrAlign = 0x0400, RxErrTooBig =
	    0x0200, RxErrSymbol = 0x0010,
			     ...
			     const struct pci_device_id *ent)
{
	...
	if (!ioaddr) {
		printk(KERN_ERR
		       "eepro100: cannot remap MMIO region %llx @ %llx\n",
	...
	if (speedo_debug > 2)
		printk
		    ("Found Intel i82557 PCI Speedo, MMIO at %#llx, IRQ %d.\n",
			 ...
			 long ioaddr, int card_idx, int acpi_idle_state)
{
	...
	if (rtdev == NULL) {
		printk(KERN_ERR
		       "eepro100: Could not allocate ethernet device.\n");
	 ...
	 */
	{
		...
		for (j = 0, i = 0, sum = 0; i < ee_size; i++) {
			u16 value =
			    do_eeprom_cmd(iobase, read_cmd | (i << 16), 27);
			eeprom[i] = value;
			sum += value;
			if (i < 3) {
...
static int do_eeprom_cmd(long ioaddr, int cmd, int cmd_len)
{
	...
	io_outw(EE_ENB, ee_addr);
	udelay(2);
	io_outw(EE_ENB | EE_SHIFT_CLK, ee_addr);
	udelay(2);

	/* Shift the command bits out. */
	do {
		short dataval =
		    (cmd & (1 << cmd_len)) ? EE_WRITE_1 : EE_WRITE_0;
		io_outw(dataval, ee_addr);
		udelay(2);
		io_outw(dataval | EE_SHIFT_CLK, ee_addr);
		udelay(2);
		retval =
		    (retval << 1) | ((io_inw(ee_addr) & EE_DATA_READ) ? 1 : 0);
	} while (--cmd_len >= 0);
	io_outw(EE_ENB, ee_addr);
	udelay(2);
...
static void speedo_resume(struct rtnet_device *rtdev)
{
	...
	/* Fill the first command with our physical address. */
	{
		...
		ias_cmd =
		    (struct descriptor *)&sp->tx_ring[sp->cur_tx++ %
						      TX_RING_SIZE];
		/* Avoid a bug(?!) here by marking the command already completed. */
		ias_cmd->cmd_status =
		    cpu_to_le32((CmdSuspend | CmdIASetup) | 0xa000);
		ias_cmd->link =
		    cpu_to_le32(TX_RING_ELEM_DMA
				(sp, sp->cur_tx % TX_RING_SIZE));
...
static void speedo_show_state(struct rtnet_device *rtdev)
{
	...
	if (speedo_debug > 0) {
		printk(KERN_DEBUG "%s: Tx ring dump,  Tx queue %u / %u:\n",
		       rtdev->name, sp->cur_tx, sp->dirty_tx);
...

	{
		...
		for (i = 0; i < 16; i++) {
			...
			if (i == 6)
				i = 21;
			printk(KERN_DEBUG
			       "%s:  PHY index %d register %d is %4.4x.\n",
			       rtdev->name, phy_num, i, mdio_read(ioaddr,
								  phy_num, i));
		}
	}
}
...
static void speedo_init_rx_ring(struct rtnet_device *rtdev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		struct rtskb *skb;
		skb =
		    rtnetdev_alloc_rtskb(rtdev,
					 PKT_BUF_SZ + 2 + sizeof(struct RxFD));
		sp->rx_skbuff[i] = skb;
		if (skb == NULL)
	...
	pci_dma_sync_single_for_device(sp->pdev,
...
static int speedo_interrupt(rtdm_irq_t * irq_handle)
{
	...
	do {
		...
		if ((status & 0x5000) ||	/* Packet received, or Rx error. */
		    (sp->rx_ring_state & (RrNoMem | RrPostponed)) ==
		    RrPostponed)
		...
		if ((sp->rx_ring_state & (RrNoMem | RrNoResources)) ==
		    RrNoResources) {
			rtdm_printk(KERN_WARNING
...
static inline struct RxFD *speedo_rx_alloc(struct rtnet_device *rtdev,
					   int entry)
{
	struct speedo_private *sp = (struct speedo_private *)rtdev->priv;
	struct RxFD *rxf;
...
static int speedo_refill_rx_buf(struct rtnet_device *rtdev, int force)
{
	...
	if (sp->rx_skbuff[entry] == NULL) {
		...
		if (rxf == NULL) {
			...
			for (forw = sp->dirty_rx + 1; forw != sp->cur_rx;
			     forw++)
...
static int speedo_close(struct rtnet_device *rtdev)
{
	...
	for (i = 0; i < TX_RING_SIZE; i++) {
		...
		if (skb) {
			pci_unmap_single(sp->pdev,
					 le32_to_cpu(sp->tx_ring[i].
						     tx_buf_addr0), skb->len,
					 PCI_DMA_TODEVICE);
...
static void set_rx_mode(struct rtnet_device *rtdev)
{
	...
	if (new_rx_mode != sp->rx_mode) {
		...
		sp->tx_ring[entry].status =
		    cpu_to_le32(CmdSuspend | CmdConfigure);
		sp->tx_ring[entry].link =
		    cpu_to_le32(TX_RING_ELEM_DMA
				(sp, (entry + 1) % TX_RING_SIZE));
	...
	if (new_rx_mode == 0) {
		...
		sp->tx_skbuff[entry] = 0;
		sp->tx_ring[entry].status =
		    cpu_to_le32(CmdSuspend | CmdMulticastList);
		sp->tx_ring[entry].link =
		    cpu_to_le32(TX_RING_ELEM_DMA
				(sp, (entry + 1) % TX_RING_SIZE));
