		    ...
		    nanosecs_abs_t * time_stamp)
{
	...
	while (!(tp->rx_ring[entry].status & cpu_to_le32(DescOwned))) {
		...
		if ((status & 0x38008300) != 0x0300) {
			...
			if (pkt_len > 1518) {
				/*RTnet */ rtdm_printk(KERN_WARNING
...

			{
				...
				if (tp->rx_buffers[entry].mapping !=
					...
					/*RTnet */ rtdm_printk(KERN_ERR
							       "%s: Internal fault: The skbuff addresses "
							       "do not match in tulip_rx: %08x vs. %08llx ? / %p.\n", rtdev->name, le32_to_cpu(tp->rx_ring[entry].buffer1), (unsigned long long)tp->rx_buffers[entry].mapping, temp);	/*RTnet */
				}
#endif
			...
			skb->protocol =
			    /*RTnet */ rt_eth_type_trans(skb, rtdev);
...
int tulip_interrupt(rtdm_irq_t * irq_handle)
{
	...
	do {
		...
		if (csr5 & (TxNoBuf | TxDied | TxIntr | TimerInt)) {
			...
			for (dirty_tx = tp->dirty_tx; tp->cur_tx - dirty_tx > 0;
				...
				int status =
				    le32_to_cpu(tp->tx_ring[entry].status);
	...
	if (tp->rx_buffers[entry].skb == NULL) {
		if (tulip_debug > 1)
			...
			rtdm_printk(KERN_WARNING
				    ...
				    rtdev->name, tp->nir, tp->cur_rx,
				    tp->ttimer, rx);
		if (tp->chip_id == LC82C168)
			outl(0x00, ioaddr + CSR7);
		else {
			if (tp->ttimer == 0
			    || (inl(ioaddr + CSR11) & 0xffff) == 0) {
				if (tulip_debug > 1)
					/*RTnet */
					rtdm_printk(KERN_WARNING
						    "%s: in rx suspend mode: (%lu) set timer\n",
						    rtdev->name, tp->nir);
				outl(tulip_tbl[tp->chip_id].
				     valid_intrs | TimerInt, ioaddr + CSR7);
	...
	if (tulip_debug > 4)
		/*RTnet */
		rtdm_printk(KERN_DEBUG "%s: exiting interrupt, csr5=%#4.4x.\n",
