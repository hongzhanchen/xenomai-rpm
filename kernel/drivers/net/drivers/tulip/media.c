...
int tulip_mdio_read(struct rtnet_device *rtdev, int phy_id, int location)
{
	...
	if (tp->chip_id == LC82C168) {
		int i = 1000;
		outl(0x60020000 + (phy_id << 23) + (location << 18),
		     ioaddr + 0xA0);
		inl(ioaddr + 0xA0);
		inl(ioaddr + 0xA0);
		while (--i > 0) {
	...
	for (i = 19; i > 0; i--) {
		...
		mdio_delay();
		retval =
		    (retval << 1) | ((inl(mdio_addr) & MDIO_DATA_READ) ? 1 : 0);
...
void tulip_mdio_write(struct rtnet_device *rtdev, int phy_id, int location,
		      int val)
{
	struct tulip_private *tp = (struct tulip_private *)rtdev->priv;
	int i;
	int cmd =
	    (0x5002 << 16) | ((phy_id & 0x1f) << 23) | (location << 18) | (val &
									   0xffff);
	long ioaddr = rtdev->base_addr;
	long mdio_addr = ioaddr + CSR9;
	unsigned long flags;
...
void tulip_select_media(struct rtnet_device *rtdev, int startup)
{
	...
	if (mtable) {
		...
		case 4:{
				...
				if (tulip_media_cap[rtdev->if_port] &
				    MediaAlwaysFD)
					tp->full_duplex = 1;
		...
		case 3:{
				...
				if (mleaf->type == 3) {	/* 21142 */
					...
					int reset_length =
					    p[2 + init_length * 2];
					misc_info =
					    reset_sequence + reset_length;
					if (startup)
						for (i = 0; i < reset_length;
						     i++)
							outl(get_u16
							     (&reset_sequence
							      [i]) << 16,
					...
					misc_info =
					    (u16 *) (reset_sequence +
						     reset_length);
					if (startup) {
						outl(mtable->csr12dir | 0x100,
						     ioaddr + CSR12);
						for (i = 0; i < reset_length;
						     i++)
							outl(reset_sequence[i],
							     ioaddr + CSR12);
					}
					for (i = 0; i < init_length; i++)
						outl(init_sequence[i],
						     ioaddr + CSR12);
				}
	...
	tp->csr6 =
	    new_csr6 | (tp->csr6 & 0xfdff) | (tp->full_duplex ? 0x0200 : 0);
...
void tulip_find_mii(struct rtnet_device *rtdev, int board_idx)
{
	...
	for (phyn = 1; phyn <= 32 && phy_idx < sizeof(tp->phys); phyn++) {
		...
		if ((mii_advert & ADVERTISE_ALL) == 0) {
			unsigned int tmpadv =
			    tulip_mdio_read(rtdev, phy, MII_BMSR);
		...
		/*RTnet */ rtdm_printk(KERN_INFO
				       ...
				       board_idx, phy, mii_reg0, mii_status,
				       mii_advert);
		...
		if (new_bmcr != mii_reg0) {
			 ...
			 */
			if (ane_switch) {
				tulip_mdio_write(rtdev, phy, MII_BMCR,
						 new_bmcr);
				udelay(10);
			}
