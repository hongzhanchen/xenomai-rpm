...
static int tulip_start_xmit(struct /*RTnet */ rtskb * skb, /*RTnet */
...
static void tulip_up( /*RTnet */ struct rtnet_device *rtdev)
{
	...
	if (tp->flags & MC_HASH_ONLY) {
		u32 addr_low =
		    cpu_to_le32(get_unaligned((u32 *) rtdev->dev_addr));
		u32 addr_high =
		    cpu_to_le32(get_unaligned((u16 *) (rtdev->dev_addr + 4)));
	...
	if (rtdev->if_port) {
		int looking_for =
		    tulip_media_cap[rtdev->if_port] & MediaIsMII ? 11 : (rtdev->
									 if_port
									 ==
									 12 ? 0
									 :
...
static void tulip_init_ring( /*RTnet */ struct rtnet_device *rtdev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		struct /*RTnet */ rtskb *skb =
		    /*RTnet */ rtnetdev_alloc_rtskb(rtdev, PKT_BUF_SZ);
		tp->rx_buffers[i].skb = skb;
		if (skb == NULL)
			break;
		mapping =
		    pci_map_single(tp->pdev, skb->tail, PKT_BUF_SZ,
				   PCI_DMA_FROMDEVICE);
...
static int
tulip_start_xmit(struct /*RTnet */ rtskb * skb, /*RTnet */
		 struct rtnet_device *rtdev)
{
	struct tulip_private *tp = (struct tulip_private *)rtdev->priv;
	int entry;
...
static int tulip_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (chip_idx == DC21040) {
		...
		for (i = 0; i < 3; i++) {
			...
			do
				value = inl(ioaddr + CSR9);
			while (value < 0 && --boguscnt > 0);
			put_unaligned(le16_to_cpu(value),
		...
		put_unaligned(inl(ioaddr + 0xA8),
			      (u16 *) (rtdev->dev_addr + 4));
		for (i = 0; i < 6; i++)
		...
		int sa_offset = 0;
		int ee_addr_size =
		    tulip_read_eeprom(ioaddr, 0xff, 8) & 0x40000 ? 8 : 6;

		for (i = 0; i < sizeof(ee_data) / 2; i++)
			((u16 *) ee_data)[i] =
			    le16_to_cpu(tulip_read_eeprom
					(ioaddr, i, ee_addr_size));
	...
	if (board_idx >= 0 && board_idx < MAX_UNITS) {
		...
		if ((options[board_idx] & FullDuplex)
		    || full_duplex[board_idx] > 0)
			tp->full_duplex = 1;
		if (mtu[board_idx] > 0)
...
{
	struct rtnet_device *rtdev =
	    (struct rtnet_device *)pci_get_drvdata(pdev);
	struct tulip_private *tp;
