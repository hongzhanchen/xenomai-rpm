			...
			struct _rt2x00_config *config)
{
	...
	rt2x00pci->eeprom_width =
	    rt2x00_get_field32(reg,
			       CSR21_TYPE_93C46) ? EEPROM_WIDTH_93c46 :
	    EEPROM_WIDTH_93c66;
	...
	for (eeprom = 0; eeprom < EEPROM_BBP_SIZE; eeprom++)
		rt2x00pci->eeprom[eeprom] =
		    rt2x00_eeprom_read_word(rt2x00pci,
...
static void
rt2x00_init_write_mac(struct _rt2x00_pci *rt2x00pci,
		      struct rtnet_device *rtnet_dev)
{
...
continue_csr_init:
	...
	for (counter = 0; counter < EEPROM_BBP_SIZE; counter++) {
		if (rt2x00pci->eeprom[counter] != 0xffff
			...
			reg_id =
			    rt2x00_get_field16(rt2x00pci->eeprom[counter],
					       EEPROM_BBP_REG_ID);
			value =
			    rt2x00_get_field16(rt2x00pci->eeprom[counter],
					       EEPROM_BBP_VALUE);
...
static int rt2x00_dev_radio_on(struct _rt2x00_core *core)
{
	...
	retval = rtdm_irq_request(&rt2x00pci->irq_handle,
				  core->rtnet_dev->irq,
				  rt2x00_interrupt, 0,
				  core->rtnet_dev->name, core->rtnet_dev);
...
static void
rt2x00_dev_update_autoresp(struct _rt2x00_pci *rt2x00pci,
			   struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_bbpsens(struct _rt2x00_pci *rt2x00pci,
			  struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_bssid(struct _rt2x00_pci *rt2x00pci,
			struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_packet_filter(struct _rt2x00_pci *rt2x00pci,
				struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_channel(struct _rt2x00_pci *rt2x00pci,
			  struct _rt2x00_config *config)
{
	...
	if (rt2x00_get_rf_value
	    (&rt2x00pci->chip, config->channel, &rt2x00pci->channel)) {
	...
	if (rt2x00_rf(&rt2x00pci->chip, RF2525)) {
		...
		rt2x00_rf_regwrite(rt2x00pci,
				   rt2x00pci->channel.rf2 +
	...
	DEBUG
	    ("Switching to channel %d. RF1: 0x%08x, RF2: 0x%08x, RF3: 0x%08x, RF4: 0x%08x.\n",
	     config->channel, rt2x00pci->channel.rf1, rt2x00pci->channel.rf2,
	     rt2x00pci->channel.rf3, rt2x00pci->channel.rf4);
}

static void
rt2x00_dev_update_rate(struct _rt2x00_pci *rt2x00pci,
		       struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_txpower(struct _rt2x00_pci *rt2x00pci,
			  struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_antenna(struct _rt2x00_pci *rt2x00pci,
			  struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_duration(struct _rt2x00_pci *rt2x00pci,
			   struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_retry(struct _rt2x00_pci *rt2x00pci,
			struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_preamble(struct _rt2x00_pci *rt2x00pci,
			   struct _rt2x00_config *config)
{
...
static void
rt2x00_dev_update_led(struct _rt2x00_pci *rt2x00pci,
		      struct _rt2x00_config *config)
{
...
int rt2x00_pci_probe(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
	...
	rtnet_dev =
	    rt2x00_core_probe(&rt2x00_pci_handler, pci_dev,
