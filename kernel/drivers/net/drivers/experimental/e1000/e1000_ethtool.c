			      ...
			      struct ethtool_drvinfo *drvinfo)
{
	...
	case e1000_ich9lan:
		sprintf(firmware_version, "%d.%d-%d",
			...
			(eeprom_data & 0x0FF0) >> 4, eeprom_data & 0x000F);
		break;
	default:
...
static int e1000_reg_test(struct e1000_adapter *adapter, u64 * data)
{
	...
	if ((mac->type != e1000_ich8lan) && (mac->type != e1000_ich9lan)) {
...
static void e1000_phy_reset_clk_and_crs(struct e1000_adapter *adapter)
{
	...
	e1000_write_phy_reg(&adapter->hw, M88E1000_EXT_PHY_SPEC_CTRL, phy_reg);
...
static int e1000_integrated_phy_loopback(struct e1000_adapter *adapter)
{
	...
	if (adapter->hw.phy.type == e1000_phy_m88) {
		...
		e1000_write_phy_reg(&adapter->hw,
				    GG82563_PHY_KMRN_MODE_CTRL, 0x1CC);
	...
	default:
		...
		if (hw->phy.type == e1000_phy_gg82563)
			e1000_write_phy_reg(hw,
					    GG82563_PHY_KMRN_MODE_CTRL, 0x180);
...
static int e1000_phys_id(struct net_device *netdev, u32 data)
{
	...
	if (adapter->hw.mac.type < e1000_82571) {
		...
		msleep_interruptible(data * 1000);
		del_timer_sync(&adapter->blink_timer);
		e1000_write_phy_reg(&(adapter->hw), IFE_PHY_SPECIAL_CONTROL_LED,
				    0);
	} else {
