...
static char version[] =
    DRV_NAME
    " for RTnet : Donald Becker and others. www.scyld.com/network/vortex.html\n";
...
enum { IS_VORTEX = 1, IS_BOOMERANG = 2, IS_CYCLONE = 4, IS_TORNADO = 8,
	...
	HAS_PWR_CTRL = 0x20, HAS_MII = 0x40, HAS_NWAY = 0x80, HAS_CB_FNS =
	    0x100,
	INVERT_MII_PWR = 0x200, INVERT_LED_PWR = 0x400, MAX_COLLISION_RESET =
	    0x800,
...
#define EISA_TBL_OFFSET	0	/* Offset of this entry for vortex_eisa_init */
	...
	PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_NWAY | HAS_HWCKSM, 128,}, {
	"3c900 Cyclone 10Mbps Combo", PCI_USES_IO | PCI_USES_MASTER,
	...
	"3c900B-FL Cyclone 10base-FL",
		    PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_HWCKSM, 128,}, {
	"3c905 Boomerang 100baseTx", PCI_USES_IO | PCI_USES_MASTER,
		    IS_BOOMERANG | HAS_MII, 64,}, {
	"3c905 Boomerang 100baseT4", PCI_USES_IO | PCI_USES_MASTER,
		    IS_BOOMERANG | HAS_MII, 64,}, {
	"3c905B Cyclone 100baseTx", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_NWAY | HAS_HWCKSM, 128,}, {
	"3c905B Cyclone 10/100/BNC", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_NWAY | HAS_HWCKSM, 128,}, {
	"3c905B-FX Cyclone 100baseFx", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_HWCKSM, 128,}, {
	"3c905C Tornado", PCI_USES_IO | PCI_USES_MASTER,
		    IS_TORNADO | HAS_NWAY | HAS_HWCKSM, 128,}, {
	"3c980 Cyclone", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_HWCKSM, 128,}, {
	"3c982 Dual Port Server Cyclone", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_HWCKSM, 128,}, {
	"3cSOHO100-TX Hurricane", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_NWAY | HAS_HWCKSM, 128,}, {
	"3c555 Laptop Hurricane", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | EEPROM_8BIT | HAS_HWCKSM, 128,}, {
	"3c556 Laptop Tornado", PCI_USES_IO | PCI_USES_MASTER,
		    IS_TORNADO | HAS_NWAY | EEPROM_8BIT | HAS_CB_FNS |
		    INVERT_MII_PWR | HAS_HWCKSM, 128,}, {
	"3c556B Laptop Hurricane", PCI_USES_IO | PCI_USES_MASTER,
		    IS_TORNADO | HAS_NWAY | EEPROM_OFFSET | HAS_CB_FNS |
		    INVERT_MII_PWR | HAS_HWCKSM, 128,}, {
	"3c575 [Megahertz] 10/100 LAN	CardBus",
		    PCI_USES_IO | PCI_USES_MASTER,
		    IS_BOOMERANG | HAS_MII | EEPROM_8BIT, 128,}, {
	"3c575 Boomerang CardBus", PCI_USES_IO | PCI_USES_MASTER,
		    IS_BOOMERANG | HAS_MII | EEPROM_8BIT, 128,}, {
	"3CCFE575BT Cyclone CardBus", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_NWAY | HAS_CB_FNS | EEPROM_8BIT |
		    INVERT_LED_PWR | HAS_HWCKSM, 128,}, {
	"3CCFE575CT Tornado CardBus", PCI_USES_IO | PCI_USES_MASTER,
		    IS_TORNADO | HAS_NWAY | HAS_CB_FNS | EEPROM_8BIT |
		    INVERT_MII_PWR | MAX_COLLISION_RESET | HAS_HWCKSM, 128,}, {
	"3CCFE656 Cyclone CardBus", PCI_USES_IO | PCI_USES_MASTER,
		    IS_CYCLONE | HAS_NWAY | HAS_CB_FNS | EEPROM_8BIT |
		    INVERT_MII_PWR | INVERT_LED_PWR | HAS_HWCKSM, 128,}, {
...
enum rx_desc_status {
	...
	IPChksumValid = 1 << 29, TCPChksumValid = 1 << 30, UDPChksumValid =
	    1 << 31,
};
...
enum tx_desc_status {
	...
	AddIPChksum = 0x02000000, AddTCPChksum = 0x04000000, AddUDPChksum =
	    0x08000000,
...
struct vortex_private {
	...
	unsigned int media_override:4,	/* Passed-in media type. */
	 ...
	 full_duplex:1, force_fd:1, autoselect:1, bus_master:1,	/* Vortex can only do a fragment bus-m. */
	 full_bus_master_tx:1, full_bus_master_rx:2,	/* Boomerang  */
	 flow_ctrl:1,		/* Use 802.3x flow control (PAUSE only) */
	 partner_flow_ctrl:1,	/* Partner supports flow control */
	 has_nway:1, enable_wol:1,	/* Wake-on-LAN is enabled */
	 pm_state_valid:1,	/* power_state[] has sane contents */
	 open:1, medialock:1, must_free_region:1;	/* Flag: if zero, Cardbus owns the I/O region */
	int drv_flags;
	u16 status_enable;
	u16 intr_enable;
...
enum xcvr_types {
	...
	XCVR_100baseFx, XCVR_MII = 6, XCVR_NWAY = 8, XCVR_ExtMII =
	    9, XCVR_Default = 10,
};
...
static void mdio_write(struct rtnet_device *vp, int phy_id, int location,
		       int value);
...
static int vortex_rx(struct rtnet_device *rtdev, int *packets,
		     nanosecs_abs_t * time_stamp);
static int boomerang_rx(struct rtnet_device *rtdev, int *packets,
			nanosecs_abs_t * time_stamp);
			 ...
			 long ioaddr, int irq, int chip_idx, int card_idx)
{
	...
	printk(KERN_INFO "%s: 3Com %s %s at 0x%lx. Vers " DRV_VERSION "\n",
	       print_name, pdev ? "PCI" : "EISA", vci->name, ioaddr);
	...
	vp->tx_ring_dma =
	    vp->rx_ring_dma + sizeof(struct boom_rx_desc) * RX_RING_SIZE;
	...
	if (print_info) {
		printk(KERN_INFO
		       "  product code %02x%02x rev %02x.%d date %02d-"
		       "%02d-%02d\n", eeprom[6] & 0xff, eeprom[6] >> 8,
		       eeprom[0x14], step, (eeprom[4] >> 5) & 15,
		       eeprom[4] & 31, eeprom[4] >> 9);
	}
...

	{
		...
		if (print_info) {
			...
			printk(KERN_INFO
			       ...
			       XCVR(config) >
			       XCVR_ExtMII ? "<invalid transceiver>" :
	...
	if (retval) {
		printk(KERN_ERR
		       "rt_3c59x: rtnet device registration failed %d\n",
		       retval);
		goto free_ring;
	}
	return 0;
...
static void issue_and_wait(struct rtnet_device *rtdev, int cmd)
{
	int i;
...
static void vortex_up(struct rtnet_device *rtdev)
{
	...
	if (vp->media_override != 7) {
		...
		if (vp->has_nway) {
			...
			while (!
			       (vp->available_media & media_tbl[rtdev->if_port].
				mask))
	...
	if (rtdev->if_port == XCVR_MII || rtdev->if_port == XCVR_NWAY) {
		...
		if (mii_reg5 == 0xffff || mii_reg5 == 0x0000) ;	/* No MII device or no link partner report */
	...
	outw(((vp->info1 & 0x8000) || vp->full_duplex ? 0x20 : 0) |
	     ...
	     ((vp->full_duplex && vp->flow_ctrl
	       && vp->partner_flow_ctrl) ? 0x100 : 0), ioaddr + Wn3_MAC_Ctrl);
...
//      issue_and_wait(dev, SetTxStart|0x07ff);
	...
	vp->status_enable =
	    SetStatusEnb | HostError | IntReq | StatsFull | TxComplete | (vp->
									  full_bus_master_tx
									  ?
									  DownComplete
									  :
									  TxAvailable)
	    | (vp->full_bus_master_rx ? UpComplete : RxComplete) | (vp->
								    bus_master ?
								    DMADone :
								    0);
	vp->intr_enable =
	    SetIntrEnb | IntLatch | TxAvailable | (vp->
						   full_bus_master_rx ? 0 :
						   RxComplete) | StatsFull |
	    HostError | TxComplete | IntReq | (vp->
					       bus_master ? DMADone : 0) |
...
static int vortex_open(struct rtnet_device *rtdev)
{
	...
	if ((retval = rtdm_irq_request(&vp->irq_handle, rtdev->irq,
				       (vp->
					full_bus_master_rx ? boomerang_interrupt
					: vortex_interrupt), 0, "rt_3c59x",
				       rtdev))) {
	...
	if (vp->full_bus_master_rx) {	/* Boomerang bus master. */
		...
		for (i = 0; i < RX_RING_SIZE; i++) {
			...
			vp->rx_ring[i].next =
			    cpu_to_le32(vp->rx_ring_dma +
					sizeof(struct boom_rx_desc) * (i + 1));
			vp->rx_ring[i].status = 0;	/* Clear complete bit. */
			vp->rx_ring[i].length =
			    cpu_to_le32(PKT_BUF_SZ | LAST_FRAG);
 ...
 */
static void
vortex_error(struct rtnet_device *rtdev, int status,
	     nanosecs_abs_t * time_stamp)
{
	struct vortex_private *vp = (struct vortex_private *)rtdev->priv;
	long ioaddr = rtdev->base_addr;
...
static int vortex_start_xmit(struct rtskb *skb, struct rtnet_device *rtdev)
{
	...
	if (vp->bus_master) {
		...
		rtdm_printk
		    ("rt_3x59x: UNSUPPORTED CODE PATH (device is lacking DMA support)!\n");
...
static int boomerang_start_xmit(struct rtskb *skb, struct rtnet_device *rtdev)
{
	...
	if (skb->ip_summed != CHECKSUM_HW)
		vp->tx_ring[entry].status =
		    cpu_to_le32(skb->len | TxIntrUploaded);
	else
		vp->tx_ring[entry].status =
		    cpu_to_le32(skb->len | TxIntrUploaded | AddTCPChksum);
...
//            }
		...
		vp->tx_ring[entry].frag[0].addr =
		    cpu_to_le32(pci_map_single
				(vp->pdev, skb->data, skb->len,
				 PCI_DMA_TODEVICE));
		vp->tx_ring[entry].frag[0].length =
		    cpu_to_le32(skb->len | LAST_FRAG);
	} else {
		int i;
		...
		for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
			...
			vp->tx_ring[entry].frag[i + 1].addr = cpu_to_le32(pci_map_single(vp->pdev,	// *** RTnet: page mapping correct? Or is this code never used?
											 (void *)page_address(frag->page) + frag->page_offset, frag->size, PCI_DMA_TODEVICE));
	...
	prev_entry->next =
	    cpu_to_le32(vp->tx_ring_dma + entry * sizeof(struct boom_tx_desc));
...
static int vortex_interrupt(rtdm_irq_t * irq_handle)
{
	...
	do {
		...
		if (status &
		    (HostError | RxEarly | StatsFull | TxComplete | IntReq)) {
			if (status == 0xffff)
				break;
		...
		if (--work_done < 0) {
			...
			do {
				vp->deferred |= status;
				outw(SetStatusEnb |
				     (~vp->deferred & vp->status_enable),
				     ioaddr + EL3_CMD);
				outw(AckIntr | (vp->deferred & 0x7ff),
				     ioaddr + EL3_CMD);
...
static int
boomerang_rx(struct rtnet_device *rtdev, int *packets,
	     nanosecs_abs_t * time_stamp)
{
	struct vortex_private *vp = (struct vortex_private *)rtdev->priv;
	...
	while ((rx_status =
		le32_to_cpu(vp->rx_ring[entry].status)) & RxDComplete) {
		if (--rx_work_limit < 0)
			break;
...
/*** RTnet ***/
			...
			{	/* Use hardware checksum info. */
				...
				if (csum_bits &&
				    (csum_bits ==
				     (IPChksumValid | TCPChksumValid)
				     || csum_bits ==
				     (IPChksumValid | UDPChksumValid))) {
...
static int vortex_close(struct rtnet_device *rtdev)
{
	...
	if (vortex_debug > 1) {
		printk(KERN_DEBUG
		       ...
		       rtdev->name, inw(ioaddr + EL3_STATUS),
		       inb(ioaddr + TxStatus));
		printk(KERN_DEBUG
		       "%s: vortex close stats: rx_nocopy %d rx_copy %d"
		       " tx_queued %d Rx pre-checksummed %d.\n", rtdev->name,
		       vp->rx_nocopy, vp->rx_copy, vp->queued_packet,
		       vp->rx_csumhits);
	}
#if DO_ZEROCOPY
	if (vp->rx_csumhits &&
...
static void dump_tx_ring(struct rtnet_device *rtdev)
{
	if (vortex_debug > 0) {
		struct vortex_private *vp =
		    (struct vortex_private *)rtdev->priv;
		long ioaddr = rtdev->base_addr;
		...
		if (vp->full_bus_master_tx) {
			...
			rtdm_printk(KERN_ERR
				    ...
				    vp->full_bus_master_tx, vp->dirty_tx,
				    vp->dirty_tx % TX_RING_SIZE, vp->cur_tx,
				    vp->cur_tx % TX_RING_SIZE);
			...
			for (i = 0; i < TX_RING_SIZE; i++) {
				rtdm_printk(KERN_ERR
					    ...
					    i, &vp->tx_ring[i],
#if DO_ZEROCOPY
					    le32_to_cpu(vp->tx_ring[i].frag[0].
							length),
#else
	...
	if (rtdev->flags & IFF_PROMISC) {
		if (vortex_debug > 0)
			printk(KERN_NOTICE "%s: Setting promiscuous mode.\n",
			       rtdev->name);
		new_mode =
		    SetRxFilter | RxStation | RxMulticast | RxBroadcast |
		    RxProm;
...
static int mdio_read(struct rtnet_device *rtdev, int phy_id, int location)
{
	...
	for (i = 14; i >= 0; i--) {
		int dataval =
		    (read_cmd & (1 << i)) ? MDIO_DATA_WRITE1 : MDIO_DATA_WRITE0;
	...
	for (i = 19; i > 0; i--) {
		...
		mdio_delay();
		retval =
		    (retval << 1) | ((inw(mdio_addr) & MDIO_DATA_READ) ? 1 : 0);
		       ...
		       int value)
{
	...
	for (i = 31; i >= 0; i--) {
		int dataval =
		    (write_cmd & (1 << i)) ? MDIO_DATA_WRITE1 :
		    MDIO_DATA_WRITE0;
		outw(dataval, mdio_addr);
		mdio_delay();
		outw(dataval | MDIO_SHIFT_CLK, mdio_addr);
