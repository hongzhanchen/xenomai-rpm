...
static void fec_enet_rx(struct rtnet_device *rtdev, int *packets,
...
#else
			rtdm_printk
			    ("%s[%d] %s: unexpected FEC_ENET_MII event\n",
 ...
 */
static void
fec_enet_rx(struct rtnet_device *rtdev, int *packets,
	    nanosecs_abs_t * time_stamp)
{
	struct fec_enet_private *fep;
	volatile fec_t *fecp;
	...
	while (!(bdp->cbd_sc & BD_ENET_RX_EMPTY)) {
		...
		/* Check for errors. */
		if (bdp->
		    cbd_sc & (BD_ENET_RX_LG | BD_ENET_RX_SH | BD_ENET_RX_NO |
...
static void mii_parse_amd79c874_dr(uint mii_reg, struct net_device *dev,
				   uint data)
{
	volatile struct fec_enet_private *fep = dev->priv;
	uint s = fep->phy_status;
...
static void mii_parse_dp83843_physts(uint mii_reg, struct net_device *dev,
				     uint data)
{
	volatile struct fec_enet_private *fep = dev->priv;
	uint s = fep->phy_status;
...
static void mii_parse_dp83846a_physts(uint mii_reg, struct net_device *dev,
				      uint data)
{
	volatile struct fec_enet_private *fep =
	    (struct fec_enet_private *)dev->priv;
	uint s = fep->phy_status;
	...
	link_change_mask =
	    PHY_STAT_LINK | PHY_STAT_10FDX | PHY_STAT_10HDX | PHY_STAT_100FDX |
	    PHY_STAT_100HDX;
...
static int fec_enet_open(struct rtnet_device *rtdev)
{
	struct fec_enet_private *fep = rtdev->priv;
	...
	if (fep->phy) {
		...
		while (fep->link == 0
		       ||
		       ((((volatile fec_t *)rtdev->base_addr)->
			 fec_ecntrl & FEC_ECNTRL_ETHER_EN) == 0)) {
			schedule();
		}
#endif /* CONFIG_IP_PNP */
...
static int fec_enet_close(struct rtnet_device *rtdev)
{
...
{
	uint retval;
	mdio_read_data_t *mrd =
	    (mdio_read_data_t *) kmalloc(sizeof(*mrd), GFP_KERNEL);
...
static int fec_enet_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	if (data == NULL) {
		...
		case SIOCDEVPRIVATE + 1:	/* for binary compat, remove in 2.5 */
			data->val_out =
			    mdio_read(dev, data->phy_id & 0x1f,
				      data->reg_num & 0x1f);
			retval = 0;
			break;
...
int __init fec_enet_init(void)
{
	...
	cbd_base =
	    (cbd_t *) consistent_alloc(GFP_KERNEL, PAGE_SIZE,
				       (void *)&mem_addr);
	...
	if ((i = rtdm_irq_request(&fep->irq_handle, FEC_INTERRUPT,
				  fec_enet_interrupt, 0, "rt_mpc8xx_fec",
				  rtdev))) {
...
#error Want to use MII, but PHY_INTERRUPT not defined!
	...
	printk
	    ("%s: FEC ENET Version 0.3, irq %d, addr %02x:%02x:%02x:%02x:%02x:%02x\n",
	     rtdev->name, FEC_INTERRUPT, rtdev->dev_addr[0], rtdev->dev_addr[1],
	     rtdev->dev_addr[2], rtdev->dev_addr[3], rtdev->dev_addr[4],
	     rtdev->dev_addr[5]);
#else
