	    ...
	    nanosecs_abs_t * time_stamp)
{
	...
	for (;;) {
		...
		if (!(bdp->cbd_sc &
		      (BD_ENET_RX_LG | BD_ENET_RX_SH | BD_ENET_RX_NO |
...
static int fcc_enet_close(struct rtnet_device *rtdev)
{
...
int __init fec_enet_init(void)
{
	...
	for (np = 0, fip = fcc_ports;
	     np < sizeof(fcc_ports) / sizeof(fcc_info_t); np++, fip++) {
		...
		if (rtdev == NULL) {
			printk(KERN_ERR
			       "fcc_enet: Could not allocate ethernet device.\n");
			return -1;
		}
		...
		printk
		    ("%s: FCC%d ENET Version 0.4, %02x:%02x:%02x:%02x:%02x:%02x\n",
		     rtdev->name, fip->fc_fccnum + 1, rtdev->dev_addr[0],
	       ...
	       volatile immap_t * immap)
{
	 ...
	 */
	cep->rx_bd_base =
	    (cbd_t *) m8260_cpm_hostalloc(sizeof(cbd_t) * RX_RING_SIZE, 8);
	ep->fen_genfcc.fcc_rbase = __pa(cep->rx_bd_base);
	cep->tx_bd_base =
	    (cbd_t *) m8260_cpm_hostalloc(sizeof(cbd_t) * TX_RING_SIZE, 8);
...
init_fcc_startup(fcc_info_t * fip, struct rtnet_device *rtdev)
{
	...
	if (rtdm_irq_request(&cep->irq_handle, fip->fc_interrupt,
			     fcc_enet_interrupt, 0, "rt_mpc8260_fcc_enet",
			     rtdev)) {
...
static int fcc_enet_open(struct rtnet_device *rtdev)
{
...
static void __exit fcc_enet_cleanup(void)
{
	...
	for (np = 0, fip = fcc_ports;
		...
		init_fcc_shutdown(fip, cep, immap);
		printk
		    ("%s: cleanup incomplete (m8260_cpm_dpfree does not exit)!\n",
		     rtdev->name);
