 ...
 *****************************************************************************/
int32_t e1000_set_phy_type(struct e1000_hw *hw)
{
...
int32_t e1000_init_hw(struct e1000_hw * hw)
{
	...
	default:
		...
		if (hw->bus_type == e1000_bus_type_pcix) {
			e1000_read_pci_cfg(hw, PCIX_COMMAND_REGISTER,
					   &pcix_cmd_word);
			...
			if (cmd_mmrbc > stat_mmrbc) {
				pcix_cmd_word &= ~PCIX_COMMAND_MMRBC_MASK;
				pcix_cmd_word |=
				    stat_mmrbc << PCIX_COMMAND_MMRBC_SHIFT;
...
static int32_t e1000_adjust_serdes_amplitude(struct e1000_hw *hw)
{
	...
	ret_val =
	    e1000_read_eeprom(hw, EEPROM_SERDES_AMPLITUDE, 1, &eeprom_data);
	if (ret_val) {
		return ret_val;
	}
...
static int32_t e1000_copper_link_ggp_setup(struct e1000_hw *hw)
{
	...
	if (!hw->phy_reset_disable) {
		 ...
		 */
		ret_val =
		    e1000_read_phy_reg(hw, GG82563_PHY_SPEC_CTRL, &phy_data);
		if (ret_val)
			return ret_val;
		...
		if (hw->disable_polarity_correction == 1)
			phy_data |= GG82563_PSCR_POLARITY_REVERSAL_DISABLE;
		ret_val =
		    e1000_write_phy_reg(hw, GG82563_PHY_SPEC_CTRL, phy_data);
	...
	if (hw->mac_type == e1000_80003es2lan) {
		...
		ret_val =
		    e1000_write_kmrn_reg(hw, E1000_KUMCTRLSTA_OFFSET_FIFO_CTRL,
		...
		ret_val =
		    e1000_read_phy_reg(hw, GG82563_PHY_SPEC_CTRL_2, &phy_data);
		if (ret_val)
			return ret_val;

		phy_data &= ~GG82563_PSCR2_REVERSE_AUTO_NEG;
		ret_val =
		    e1000_write_phy_reg(hw, GG82563_PHY_SPEC_CTRL_2, phy_data);
		...
		if (e1000_check_mng_mode(hw) == FALSE) {
			...
			ret_val =
			    e1000_write_phy_reg(hw, GG82563_PHY_PWR_MGMT_CTRL,
						phy_data);
			if (ret_val)
				return ret_val;
...
* hw - Struct containing variables accessed by shared code
*********************************************************************/
static int32_t e1000_copper_link_ife_setup(struct e1000_hw *hw)
{
	if (hw->phy_reset_disable)
		return E1000_SUCCESS;
	return E1000_SUCCESS;
...
static int32_t e1000_setup_copper_link(struct e1000_hw *hw)
{
	...
	case e1000_ich8lan:
		...
		ret_val =
		    e1000_write_kmrn_reg(hw, GG82563_REG(0x34, 4), 0xFFFF);
		if (ret_val)
			return ret_val;
		ret_val =
		    e1000_read_kmrn_reg(hw, GG82563_REG(0x34, 9), &reg_data);
		if (ret_val)
			return ret_val;
		reg_data |= 0x3F;
		ret_val =
		    e1000_write_kmrn_reg(hw, GG82563_REG(0x34, 9), reg_data);
		if (ret_val)
			return ret_val;
	default:
		break;
	...
	case e1000_80003es2lan:
		...
		ret_val =
		    e1000_write_kmrn_reg(hw, E1000_KUMCTRLSTA_OFFSET_INB_CTRL,
					 reg_data);
		if (ret_val)
			return ret_val;
		break;
...
int32_t e1000_phy_setup_autoneg(struct e1000_hw * hw)
{
	...
	if (hw->autoneg_advertised & ADVERTISE_1000_FULL) {
		...
		if (hw->phy_type == e1000_phy_ife) {
			DEBUGOUT
			    ("e1000_phy_ife is a 10/100 PHY. Gigabit speed is not supported.\n");
		}
	}
...
static int32_t e1000_phy_force_speed_duplex(struct e1000_hw *hw)
{
	...
	if ((hw->phy_type == e1000_phy_m88) ||
		...
		ret_val =
		    e1000_read_phy_reg(hw, IFE_PHY_MDIX_CONTROL, &phy_data);
		if (ret_val)
			return ret_val;
		...
		ret_val =
		    e1000_write_phy_reg(hw, IFE_PHY_MDIX_CONTROL, phy_data);
		if (ret_val)
			return ret_val;
	} else {
	...
	if (hw->phy_type == e1000_phy_m88) {
		...
		ret_val =
		    e1000_read_phy_reg(hw, GG82563_PHY_MAC_SPEC_CTRL,
				       &phy_data);
		if (ret_val)
			return ret_val;
...
int32_t e1000_check_for_link(struct e1000_hw * hw)
{
	...
	if ((hw->media_type == e1000_media_type_internal_serdes) &&
	    (E1000_TXCW_ANE & E1000_READ_REG(hw, TXCW))) {
		hw->serdes_link_down =
		    !(E1000_STATUS_LU & E1000_READ_REG(hw, STATUS));
	}
	return E1000_SUCCESS;
}
...
int32_t e1000_kumeran_lock_loss_workaround(struct e1000_hw * hw)
{
	...
	if (phy_data & MII_SR_LINK_STATUS) {
		for (cnt = 0; cnt < 10; cnt++) {
			/* read once to clear */
			ret_val =
			    e1000_read_phy_reg(hw, IGP3_KMRN_DIAG, &phy_data);
			if (ret_val)
				return ret_val;
...
e1000_phy_m88_get_info(struct e1000_hw * hw, struct e1000_phy_info * phy_info)
{
	...
	if ((phy_data & M88E1000_PSSR_SPEED) == M88E1000_PSSR_1000MBS) {
		...
		if (hw->phy_type != e1000_phy_gg82563) {
			...
			ret_val =
			    e1000_read_phy_reg(hw, GG82563_PHY_DSP_DISTANCE,
					       &phy_data);
			if (ret_val)
				return ret_val;

			phy_info->cable_length =
			    phy_data & GG82563_DSPD_CABLE_LENGTH;
		}
...
int32_t e1000_init_eeprom_params(struct e1000_hw * hw)
{
	...
	case e1000_ich8lan:
		{
			int32_t i = 0;
			uint32_t flash_size =
			    E1000_READ_ICH8_REG(hw, ICH8_FLASH_GFPREG);
			...
			hw->flash_base_addr =
			    (flash_size & ICH8_GFPREG_BASE_MASK) *
			    ICH8_FLASH_SECTOR_SIZE;

			hw->flash_bank_size =
			    ((flash_size >> 16) & ICH8_GFPREG_BASE_MASK) + 1;
			hw->flash_bank_size -=
			    (flash_size & ICH8_GFPREG_BASE_MASK);
	...
	if (eeprom->type == e1000_eeprom_spi) {
		...
		if (hw->mac_type <= e1000_82547_rev_2) {
			...
			eeprom->word_size = 64;
			ret_val =
			    e1000_read_eeprom(hw, EEPROM_CFG, 1, &eeprom_size);
			if (ret_val)
				return ret_val;
			eeprom_size =
			...
			eeprom_size =
			    (uint16_t) ((eecd & E1000_EECD_SIZE_EX_MASK) >>
 ...
 * hw - Struct containing variables accessed by shared code
 *****************************************************************************/
int32_t e1000_spi_eeprom_ready(struct e1000_hw *hw)
{
		       ...
		       uint16_t offset, uint16_t words, uint16_t * data)
{
	...
	for (i = 0; i < words; i++) {
		...
		data[i] =
		    (E1000_READ_REG(hw, EERD) >> E1000_EEPROM_RW_REG_DATA);
		...
		do {
			...
			for (i = 0; i < E1000_SHADOW_RAM_WORDS; i++) {
				...
				if (hw->eeprom_shadow_ram[i].modified == TRUE) {
					low_byte =
					    (uint8_t) hw->eeprom_shadow_ram[i].
					    eeprom_word;
				...
				if (i == E1000_ICH8_NVM_SIG_WORD)
					high_byte =
					    E1000_ICH8_NVM_SIG_MASK | high_byte;
...
uint32_t e1000_hash_mc_addr(struct e1000_hw *hw, uint8_t * mc_addr)
{
	...
	case 3:
		if (hw->mac_type == e1000_ich8lan) {
			...
			hash_value =
			    ((mc_addr[4]) | (((uint16_t) mc_addr[5]) << 8));
		}
		break;
	}
	...
	if ((hw->phy_type == e1000_phy_m88) ||
		...
		*polarity = (phy_data & M88E1000_PSSR_REV_POLARITY) >>
		...
		if ((phy_data & IGP01E1000_PSSR_SPEED_MASK) ==
			...
			*polarity =
			    (phy_data & IGP01E1000_PHY_POLARITY_MASK) ? 1 : 0;
		} else {
		...
		ret_val =
		    e1000_read_phy_reg(hw, IFE_PHY_EXTENDED_STATUS_CONTROL,
				       &phy_data);
		if (ret_val)
			return ret_val;
...
int32_t
e1000_config_dsp_after_link_change(struct e1000_hw * hw, boolean_t link_up)
{
	int32_t ret_val;
 ...
 ****************************************************************************/
int32_t e1000_host_if_read_cookie(struct e1000_hw * hw, uint8_t * buffer)
{
	uint8_t i;
			   ...
			   struct e1000_host_mng_command_header * hdr)
{
	...
	for (i = 0; i < length; i++) {
		E1000_WRITE_REG_ARRAY_DWORD(hw, HOST_IF, i,
					    *((uint32_t *) hdr + i));
		E1000_WRITE_FLUSH(hw);
	}
			  ...
			  uint16_t length)
{
	...
	if (ret_val == E1000_SUCCESS) {
		ret_val =
		    e1000_mng_host_if_write(hw, buffer, length, sizeof(hdr),
					    &(hdr.checksum));
		if (ret_val == E1000_SUCCESS) {
 ...
 ****************************************************************************/
uint8_t e1000_calculate_mng_checksum(char *buffer, uint32_t length)
{
	uint8_t sum = 0;
	uint32_t i;
...
int32_t e1000_ife_disable_dynamic_power_down(struct e1000_hw * hw)
{
	...
	if (hw->phy_type == e1000_phy_ife) {
		ret_val =
		    e1000_read_phy_reg(hw, IFE_PHY_SPECIAL_CONTROL, &phy_data);
		if (ret_val)
			return ret_val;

		phy_data |= IFE_PSC_DISABLE_DYNAMIC_POWER_DOWN;
		ret_val =
		    e1000_write_phy_reg(hw, IFE_PHY_SPECIAL_CONTROL, phy_data);
	}
...
int32_t e1000_ich8_cycle_init(struct e1000_hw * hw)
{
	...
	if (hsfsts.hsf_status.flcinprog == 0) {
		...
		for (i = 0; i < ICH8_FLASH_COMMAND_TIMEOUT; i++) {
			hsfsts.regval =
			    E1000_READ_ICH8_REG16(hw, ICH8_FLASH_HSFSTS);
			if (hsfsts.hsf_status.flcinprog == 0) {
				error = E1000_SUCCESS;
				break;
			}
		...
		if (error == E1000_SUCCESS) {
			...
			hsfsts.hsf_status.flcdone = 1;
			E1000_WRITE_ICH8_REG16(hw, ICH8_FLASH_HSFSTS,
					       hsfsts.regval);
		} else {
		     ...
		     uint32_t size, uint16_t * data)
{
	...
	do {
		...
		E1000_WRITE_ICH8_REG(hw, ICH8_FLASH_FADDR,
...
int32_t e1000_erase_ich8_4k_segment(struct e1000_hw * hw, uint32_t segment)
{
	...
	for (j = 0; j < iteration; j++) {
		do {
			 ...
			 * Control */
			hsflctl.regval =
			    E1000_READ_ICH8_REG16(hw, ICH8_FLASH_HSFCTL);
			hsflctl.hsf_ctrl.flcycle = ICH8_CYCLE_ERASE;
			E1000_WRITE_ICH8_REG16(hw, ICH8_FLASH_HSFCTL,
					       hsflctl.regval);
			...
			/* TBD */
			flash_linear_address =
			    segment * sector_size + j * seg_size;
				      ...
				      uint32_t cnf_base_addr, uint32_t cnf_size)
{
	...
	for (i = 0; i < cnf_size; i++) {
		ret_val =
		    e1000_read_eeprom(hw, (word_addr + i * 2), 1, &reg_data);
		if (ret_val)
			return ret_val;

		ret_val =
		    e1000_read_eeprom(hw, (word_addr + i * 2 + 1), 1,
				      &reg_addr);
		if (ret_val)
			return ret_val;
		...
		ret_val =
		    e1000_write_phy_reg_ex(hw, (uint32_t) reg_addr, reg_data);
...
int32_t e1000_init_lcd_from_nvm(struct e1000_hw * hw)
{
	...
	if ((reg_data & E1000_EXTCNF_CTRL_LCD_WRITE_ENABLE) == 0x0000) {
		...
		if (cnf_size) {
			reg_data = E1000_READ_REG(hw, EXTCNF_CTRL);
			cnf_base_addr =
			    reg_data & E1000_EXTCNF_CTRL_EXT_CNF_POINTER;
			/* cnf_base_addr is in DWORD */
			cnf_base_addr >>= 16;
