...
//======================================================================================================
//======================================================================================================
static int rtl8169_init_board(struct pci_dev *pdev,
			      struct rtnet_device **dev_out,
			      unsigned long *ioaddr_out, int region)
{
	...
	*dev_out = NULL;
	...
	{
		unsigned char val8 =
		    (unsigned char)(RTL8169_READ_GMII_REG(ioaddr, 3) & 0x000f);
		if (val8 == 0x00) {
...
//#endif //end #ifdef RTL8169_JUMBO_FRAME_SUPPORT
	...
	printk(KERN_INFO "%s: %s at 0x%lx, "
	       "%2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x, " "IRQ %d\n", rtdev->name,
	       RTL8169_DRIVER_NAME, rtdev->base_addr, rtdev->dev_addr[0],
	       rtdev->dev_addr[1], rtdev->dev_addr[2], rtdev->dev_addr[3],
	       rtdev->dev_addr[4], rtdev->dev_addr[5], rtdev->irq);
	...
	if (!(RTL_R8(PHYstatus) & TBI_Enable)) {
		...
		if (option > 0) {
			...
			case _100_Half:
				Cap10_100 =
				    PHY_Cap_100_Half | PHY_Cap_10_Full |
				    PHY_Cap_10_Half;
				Cap1000 = PHY_Cap_Null;
				break;
			case _100_Full:
				Cap10_100 =
				    PHY_Cap_100_Full | PHY_Cap_100_Half |
				    PHY_Cap_10_Full | PHY_Cap_10_Half;
				Cap1000 = PHY_Cap_Null;
				break;
			case _1000_Full:
				Cap10_100 =
...
//                      RTL8169_WRITE_GMII_REG( ioaddr, PHY_1000_CTRL_REG, PHY_Cap_1000_Full );
		...
		if (option & _1000bpsF) {
			...
			if (option & _100bps) {
				priv->linkstatus =
				    (option & FullDup) ? _100_Full : _100_Half;
			} else {
				priv->linkstatus =
				    (option & FullDup) ? _10_Full : _10_Half;
			}
		}
...
//      u32 TxPhyAddr, RxPhyAddr;
	...
	retval =
	    rtdm_irq_request(&priv->irq_handle, rtdev->irq, rtl8169_interrupt,
			     0, "rt_r8169", rtdev);
	/*** /RTnet ***/
	...
	priv->txdesc_space =
	    pci_alloc_consistent(pdev, priv->sizeof_txdesc_space,
	...
	priv->rxdesc_space =
	    pci_alloc_consistent(pdev, priv->sizeof_rxdesc_space,
	...
	if (priv->txdesc_phy_dma_addr & 0xff) {
		printk
		    ("%s: Gigabit driver txdesc_phy_dma_addr is not 256-bytes-aligned.\n",
		     rtdev->name);
	}
	if (priv->rxdesc_phy_dma_addr & 0xff) {
		printk
		    ("%s: Gigabit driver rxdesc_phy_dma_addr is not 256-bytes-aligned.\n",
		     rtdev->name);
	}
...

	{
		...
		for (i = 0; i < NUM_RX_DESC; i++) {
			...
			skb = rtnetdev_alloc_rtskb(rtdev, priv->rx_buf_size);
			    /*** RTnet ***/ ;
			if (skb != NULL) {
				...
				printk
				    ("%s: Gigabit driver failed to allocate skbuff.\n",
				     rtdev->name);
				priv->drvinit_fail = 1;
			}
		}
...
static void rtl8169_hw_PHY_config(struct rtnet_device *rtdev)
{
	...
	} else if ((priv->mcfg == MCFG_METHOD_2)
		   || (priv->mcfg == MCFG_METHOD_3)) {
		RTL8169_WRITE_GMII_REG((unsigned long)ioaddr, 0x1F, 0x0001);
...
static void rtl8169_init_ring(struct rtnet_device *rtdev)
{
	...
	for (i = 0; i < NUM_TX_DESC; i++) {
		...
		priv->txdesc_array_dma_addr[i] =
		    pci_map_single(pdev, &priv->TxDescArray[i],
				   sizeof(struct TxDesc), PCI_DMA_TODEVICE);
	}

	for (i = 0; i < NUM_RX_DESC; i++) {
		if (i == (NUM_RX_DESC - 1)) {
			priv->RxDescArray[i].status =
			    cpu_to_le32((OWNbit | EORbit) | (unsigned long)
					priv->hw_rx_pkt_len);
		} else {
			priv->RxDescArray[i].status =
			    cpu_to_le32(OWNbit | (unsigned long)priv->
					hw_rx_pkt_len);
		}
		...
		{		//-----------------------------------------------------------------------
			...
			if (skb != NULL) {
				priv->RxDescArray[i].buf_addr =
				    cpu_to_le32(priv->rx_skbuff_dma_addr[i]);
		...
		priv->rxdesc_array_dma_addr[i] =
		    pci_map_single(pdev, &priv->RxDescArray[i],
				   sizeof(struct RxDesc), PCI_DMA_TODEVICE);
...
static int rtl8169_start_xmit(struct rtskb *skb, struct rtnet_device *rtdev)
{
	...
	if ((status & OWNbit) == 0) {
		...
		if (len < ETH_ZLEN) {
			...
			if (skb == NULL) {
				/* Error... */
				rtdm_printk
				    ("%s: Error -- rtskb_padto returned NULL; out of memory?\n",
				     rtdev->name);
			}
			len = ETH_ZLEN;
		}
		...
		txbuf_dma_addr =
		    pci_map_single(pdev, skb->data, len, PCI_DMA_TODEVICE);
		...
		while (r8169_debug & (DEBUG_TX_SYNC | DEBUG_TX_OTHER)) {
			...
			RT_DBG_PRINT
			    ("%s: TX len = %d, skb->len = %d, eth_proto=%04x\n",
		...
		if (len > priv->tx_pkt_len) {
			rtdm_printk
			    ("%s: Error -- Tx packet size(%d) > mtu(%d)+14\n",
	...
	if ((priv->cur_tx - NUM_TX_DESC) == priv->dirty_tx) {
		if (r8169_debug & DEBUG_RUN)
			rtdm_printk(KERN_DEBUG "%s: stopping rtnetif queue",
				    __FUNCTION__);
...
/* This routine is logically part of the interrupt handler, but isolated
   for clarity. */
static void rtl8169_tx_interrupt(struct rtnet_device *rtdev,
				 struct rtl8169_private *priv,
				 unsigned long ioaddr)
{
	...
	while ((tx_left > 0) && (txloop_cnt < max_interrupt_work)) {
		...
		if ((le32_to_cpu(priv->TxDescArray[entry].status) & OWNbit) ==
		    0) {

#ifdef RTL8169_DYNAMIC_CONTROL
			r8169_callback_tx(&(priv->rt), 1,
...
/* This routine is logically part of the interrupt handler, but isolated
   for clarity. */
static void rtl8169_rx_interrupt(struct rtnet_device *rtdev,
				 struct rtl8169_private *priv,
				 unsigned long ioaddr,
				 nanosecs_abs_t * time_stamp)
{
	...
	pci_dma_sync_single_for_cpu(pdev, priv->rxdesc_array_dma_addr[cur_rx],
				    sizeof(struct RxDesc), PCI_DMA_FROMDEVICE);

	while (((le32_to_cpu(rxdesc->status) & OWNbit) == 0)
		...
		if (le32_to_cpu(rxdesc->status) & RxRES) {
			...
			pkt_size =
			    (int)(le32_to_cpu(rxdesc->status) & 0x00001FFF) - 4;

			if (pkt_size > priv->rx_pkt_len) {
				rtdm_printk
				    ("%s: Error -- Rx packet size(%d) > mtu(%d)+14\n",
				...
				if (n_skb != NULL) {
					...
					if (rx_skb != NULL) {
						pci_dma_sync_single_for_cpu
						    (pdev,
						     priv->
						     rx_skbuff_dma_addr[cur_rx],
						     sizeof(struct RxDesc),
						     PCI_DMA_FROMDEVICE);

						rtskb_put(rx_skb, pkt_size);
						rx_skb->protocol =
						    rt_eth_type_trans(rx_skb,
								      rtdev);
...
//                                              dev->last_rx = jiffies;
					...
					RT_DBG_PRINT
					    ("%s: Allocate n_skb failed! (priv->rx_buf_size = %d)\n",
				...
				if (cur_rx == (NUM_RX_DESC - 1)) {
					...
					priv->RxDescArray[cur_rx].status =
					    cpu_to_le32(OWNbit | (unsigned long)
							priv->hw_rx_pkt_len);
				}
				...
				if (cur_skb != NULL) {
					priv->rx_skbuff_dma_addr[cur_rx] =
					    pci_map_single(pdev, cur_skb->data,
							   priv->
							   rx_buf_size
							   /* <kk> MAX_RX_SKBDATA_SIZE */
							   ,
							   PCI_DMA_FROMDEVICE);
					rxdesc->buf_addr =
					    cpu_to_le32(priv->
		...
		pci_dma_sync_single_for_cpu(pdev,
					    priv->rxdesc_array_dma_addr[cur_rx],
					    sizeof(struct RxDesc),
					    PCI_DMA_FROMDEVICE);
...
static int __init rtl8169_init_module(void)
{
	...
	if (r8169_debug & DEBUG_RUN)
		printk("Initializing " MODULENAME " driver");
