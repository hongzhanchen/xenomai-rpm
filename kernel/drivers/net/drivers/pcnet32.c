...
static int pcnet32_open(struct rtnet_device *dev)
{
/*** RTnet ***/
...
static int pcnet32_init_ring(struct rtnet_device *dev)
{
/*** RTnet ***/
	struct pcnet32_private *lp = dev->priv;
	int i;
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		struct rtskb *rx_skbuff = lp->rx_skbuff[i];
						    /*** RTnet ***/
		if (rx_skbuff == NULL) {
			if (!(rx_skbuff = lp->rx_skbuff[i] = rtnetdev_alloc_rtskb(dev, PKT_BUF_SZ))) {
												/*** RTnet ***/
			...
			rtskb_reserve(rx_skbuff, 2);
					  /*** RTnet ***/
		}
		lp->rx_dma_addr[i] =
		    pci_map_single(lp->pci_dev, rx_skbuff->tail, rx_skbuff->len,
				   PCI_DMA_FROMDEVICE);
...
static int pcnet32_start_xmit(struct rtskb *skb, struct rtnet_device *dev)
{
/*** RTnet ***/
	struct pcnet32_private *lp = dev->priv;
	unsigned long ioaddr = dev->base_addr;
 ...
 *** RTnet ***/
	...
	lp = dev->priv;
	old_packet_cnt = lp->stats.rx_packets;
					   /*** RTnet ***/
	...
	while ((csr0 = lp->a.read_csr(ioaddr, 0)) & 0x8600 && --boguscnt >= 0) {
		...
		if (csr0 & 0x0200) {	/* Tx-done interrupt */
			...
			if (lp->tx_full && rtnetif_queue_stopped(dev) &&
					      /*** RTnet ***/
...
static int pcnet32_rx(struct rtnet_device *dev, nanosecs_abs_t * time_stamp)
{
/*** RTnet ***/
	struct pcnet32_private *lp = dev->priv;
...
static int pcnet32_close(struct rtnet_device *dev)
{
/*** RTnet ***/
	unsigned long ioaddr = dev->base_addr;
	struct pcnet32_private *lp = dev->priv;
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		if (lp->rx_skbuff[i]) {
			pci_unmap_single(lp->pci_dev, lp->rx_dma_addr[i],
					 lp->rx_skbuff[i]->len,
					 PCI_DMA_FROMDEVICE);
			dev_kfree_rtskb(lp->rx_skbuff[i]);
					       /*** RTnet ***/
		}
		lp->rx_skbuff[i] = NULL;
		lp->rx_dma_addr[i] = 0;
	...
	for (i = 0; i < TX_RING_SIZE; i++) {
		if (lp->tx_skbuff[i]) {
			pci_unmap_single(lp->pci_dev, lp->tx_dma_addr[i],
					 lp->tx_skbuff[i]->len,
					 PCI_DMA_TODEVICE);
			dev_kfree_rtskb(lp->tx_skbuff[i]);
					       /*** RTnet ***/
		}
		lp->tx_skbuff[i] = NULL;
		lp->tx_dma_addr[i] = 0;
