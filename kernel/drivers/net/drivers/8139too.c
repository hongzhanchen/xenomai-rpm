...
const static struct {
	...
	{
	"RTL-8139", 0x40, HasHltClk,}, {
	"RTL-8139 rev K", 0x60, HasHltClk,}, {
		"RTL-8139A", 0x70, HasHltClk,	/* XXX undocumented? */
	}, {
		"RTL-8139A rev G", 0x72, HasHltClk,	/* XXX undocumented? */
	}, {
	"RTL-8139B", 0x78, HasLWake,}, {
	"RTL-8130", 0x7C, HasLWake,}, {
	"RTL-8139C", 0x74, HasLWake,}, {
	"RTL-8100", 0x7A, HasLWake,}, {
		"RTL-8100B/8139D", 0x75, HasHltClk	/* XXX undocumented? */
	| HasLWake,}, {
"RTL-8101", 0x77, HasLWake,},};
			    ...
			    const struct pci_device_id *ent)
{
	...
	if (tp->default_port) {
		...
		mdio_write(rtdev, tp->phys[0], 0, ((option & 0x20) ? 0x2000 : 0) |	/* 100Mbps? */
...
static int mdio_read(struct rtnet_device *rtdev, int phy_id, int location)
{
	...
	for (i = 19; i > 0; i--) {
		...
		mdio_delay(mdio_addr);
		retval =
		    (retval << 1) | ((readb(mdio_addr) & MDIO_DATA_IN) ? 1 : 0);
...
static int rtl8139_open(struct rtnet_device *rtdev)
{
	...
	tp->tx_bufs =
	    pci_alloc_consistent(tp->pci_dev, TX_BUF_TOT_LEN, &tp->tx_bufs_dma);
	tp->rx_ring =
	    pci_alloc_consistent(tp->pci_dev, RX_BUF_TOT_LEN, &tp->rx_ring_dma);
...
static void rtl8139_rx_err
    (u32 rx_status, struct rtnet_device *rtdev, struct rtl8139_private *tp,
     void *ioaddr) {
/*        u8 tmp8;
				 ...
				 nanosecs_abs_t * time_stamp)
{
	...
	while ((RTL_R8(ChipCmd) & RxBufEmpty) == 0) {
		...
		if ((rx_size > (MAX_ETH_FRAME_SIZE + 4)) ||
		    (rx_size < 8) || (!(rx_status & RxStatusOK))) {
...
static int rtl8139_interrupt(rtdm_irq_t * irq_handle)
{
	...
	if (unlikely(status == 0xFFFF)
	    || unlikely(!(status & rtl8139_intr_mask)))
		goto out;
...
static int rtl8139_close(struct rtnet_device *rtdev)
{
	...
	pci_free_consistent(tp->pci_dev, RX_BUF_TOT_LEN, tp->rx_ring,
			    tp->rx_ring_dma);
	pci_free_consistent(tp->pci_dev, TX_BUF_TOT_LEN, tp->tx_bufs,
			    tp->tx_bufs_dma);
	tp->rx_ring = NULL;
	tp->tx_bufs = NULL;
